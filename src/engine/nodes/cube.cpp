...
CubeNode::CubeNode() {
    ...
    params.addFloat("size", "Size", 1.0f, 0.1f, 5.0f, 0.1f);
...
void CubeNode::applyParams() {
    Param* p;
    if ((p = params.find("size")))        cubeSize = p->value;
...
void CubeNode::syncParams() {
    params.set("size", cubeSize);
...
void CubeNode::regenerate() {
    if (cubeSize == m_lastSize && !m_triangles.empty()) return;
    ObjLoader::generateCube(m_triangles, cubeSize);
    m_lastSize = cubeSize;
}
...
void CubeNode::renderMesh(Renderer& r) {
...
void CubeNode::render(Renderer& r) {
    if (!active) return;
    renderMesh(r);
}
